...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="mb-0">Edit Coupon: {{ $coupon->coupon_code }}</h4>
                    ...
                    <div class="card-body">
                        <div class="row mb-3">
                            <div class="col-md-8">
                                ...
                                <input type="text" name="coupon_code" id="coupon_code" class="form-control text-uppercase" placeholder="e.g. SAVE20" value="{{ old('coupon_code', $coupon->coupon_code) }}" required>
                            </div>
                            <div class="col-md-4">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" step="0.01" name="discount_value" id="discount_value" class="form-control" placeholder="Value" value="{{ old('discount_value', $coupon->discount_value) }}" required min="0">
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6" id="maxDiscountContainer">
                                ...
                                <input type="number" step="0.01" name="max_discount" id="max_discount" class="form-control" placeholder="Max Deduction Limit" value="{{ old('max_discount', $coupon->max_discount) }}" min="0">
                            </div>
                            <div class="col-md-6">
                                <label for="minimum_order_amount" class="form-label fw-bold">Min Order Amount (₹)</label>
                                <input type="number" step="0.01" name="minimum_order_amount" id="minimum_order_amount" class="form-control" placeholder="e.g. 1000" value="{{ old('minimum_order_amount', $coupon->minimum_order_amount) }}" min="0">
                            </div>
                        </div>
                            ...
                            <div class="input-group">
                                ...
                                @php
                                    ...
                                    if ($coupon->valid_from && $coupon->valid_to) {
                                        $rangeVal = $coupon->valid_from->format('Y-m-d') . ' to ' . $coupon->valid_to->format('Y-m-d');
                                    } elseif ($coupon->valid_from) {
                                        $rangeVal = $coupon->valid_from->format('Y-m-d');
                                    }
                                @endphp
                                <input type="text" name="validity_range" id="validity_range" class="form-control" placeholder="Select date range..." value="{{ old('validity_range', $rangeVal) }}">
                            </div>
                    ...
                    <div class="card-body">
                        <div class="targeting-section">
                            <div class="form-check mb-2">
                                <input class="form-check-input audience-radio" type="radio" name="audience_type" id="audience_all" value="all" {{ old('audience_type', $coupon->is_public ? 'all' : 'specific') === 'all' ? 'checked' : '' }}>
                                <label class="form-check-label fw-bold" for="audience_all">
                                    All Customers (Current & Past 5 Months Registered)
                                </label>
                            </div>
                            <div class="form-check mb-2">
                                <input class="form-check-input audience-radio" type="radio" name="audience_type" id="audience_specific" value="specific" {{ old('audience_type', $coupon->is_public ? 'all' : 'specific') === 'specific' ? 'checked' : '' }}>
                        ...
                        <div class="audience-card" id="specificCustomersPanel" style="{{ !$coupon->is_public ? 'display: block;' : '' }}">
                            ...
                            <div class="user-scroll-list" id="specificCustomersList">
                                @if(!$coupon->is_public && count($assignedUsers) > 0)
                                    @foreach($assignedUsers as $user)
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="checkbox" name="user_ids[]" value="{{ $user->id }}" id="user_{{ $user->id }}" checked>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Keep initial state according to current configuration
        ...
        searchCustomerBtn.addEventListener('click', function() {
            ...
            // Keep the already checked elements so they aren't lost
            const checkedIds = Array.from(document.querySelectorAll('input[name="user_ids[]"]:checked')).map(cb => cb.value);
            ...
            fetch(`{{ route('admin.coupons.search-users') }}?type=specific&query=${encodeURIComponent(query)}`)
                ...
                .then(data => {
                    ...
                    data.users.forEach(user => {
                        const isChecked = checkedIds.includes(user.id.toString()) ? 'checked' : '';
